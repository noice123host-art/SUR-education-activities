...
screenGui.Parent = ProtectGui -- ĐƯA VÀO COREGUI
...
local function FlyTo(targetCF)
    ...
    -- Kiểm tra nhân vật phải còn sống
    ...
    while task.wait(0.6) do
        if _G.AutoRoll then
            pcall(function()
                ...
                if not char then return end
                local hum = char:FindFirstChild("Humanoid")
                -- Bỏ qua thao tác này nếu nhân vật đang chết
                ...
                if tool and hum then
                    ...
                    if toolName == "Rokakaka" then 
                        task.wait(0.4)
                        -- Tìm kiếm nút bấm trong UI của game
    ...
    while task.wait(0.2) do
        if _G.MainFarm then
            pcall(function()
                ...
                local hrp = char:FindFirstChild("HumanoidRootPart")
                local hum = char:FindFirstChild("Humanoid")
                -- Nếu đang chết thì không làm gì, chờ tự dọn dẹp
                if not hrp or not hum or hum.Health <= 0 then return end
